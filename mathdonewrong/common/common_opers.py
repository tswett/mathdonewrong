...
class Compose(NamedOper):
    ...
    Generally, if `f` is something of type :math:`A \to B` and `g` is something
    of type :math:`B \to C`, then `Compose(f, g)` is something of type :math:`A
    \to C`.
